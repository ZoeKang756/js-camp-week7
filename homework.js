...
1. Fetch (原生 API) 不需要另外引入外部套件, 
   Axios 是一個輕量級的第三方函式庫，封裝了 XMLHttpRequest (XHR) , 
         需要在專案中安裝 (npm install axios)

2. Fetch：不會自動將 HTTP 狀態碼錯誤（如 404 或 500）視為 Promise 的 rejection。
         如果網路請求成功，即使回應是 500，fetch 的 Promise 仍會 resolve，
         開發者需要手動檢查 response.ok 或 response.status。
   Axios：會自動將狀態碼不在 2xx 範圍內的情形視為錯誤並拋出異常，開發者只需使用 catch 捕獲即可。

3. Fetch：需要手動調用 .json() 方法來將回應內容轉化為 JSON 物件。
   Axios：會自動將回應資料（Data）轉換為 JSON 物件，開發者可以直接使用。
*/
